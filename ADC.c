...
#define PIN_Ambient (2)// PORT B 
#define PIN_Flame (3)// PORT B 
...
int adcSelect(int pin){
    channel_pin=-1;
  
    if(pin==PIN_Ambient){
        SIM->SCGC5 |= SIM_SCGC5_PORTB_MASK;// enable clock
        PORTB->PCR[PIN_Ambient] &= ~PORT_PCR_MUX_MASK;// analog Input
        channel_pin=12; 
    }
    if(pin==PIN_Flame){
        SIM->SCGC5 |= SIM_SCGC5_PORTB_MASK;// enable clock
        PORTB->PCR[PIN_Flame]&= ~PORT_PCR_MUX_MASK;// analog Input
        channel_pin=13;   
    }
